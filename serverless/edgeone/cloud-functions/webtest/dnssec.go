...
// 通过 DoH 查询 DNSKEY、A、DS 记录，用 RRSIG.Verify() 验证签名
// 流程：
...
func ResolveDNSSEC(domain string) (*DNSSECResult, error) {
	...
	responseDNSKEY, _, err := executeDoHQueryMsg(msgDNSKEY)
	...
	responseA, _, err := executeDoHQueryMsg(msgA)
	...
	responseDS, _, _ := executeDoHQueryMsg(msgDS)
...
func ResolveDNSSECForRecord(domain string, recordType uint16) (*DNSSECResult, error) {
	...
	response, _, err := executeDoHQueryMsg(msg)
	result.Duration = time.Since(start).Seconds() * 1000
