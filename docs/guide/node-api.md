...
```
GET /            → {"status":"ok","version":"v1.2.3","capabilities":["probe","report","config","ota"]}
```

无鉴权、不计入统计。除存活外同时回报**版本号**与**能力清单**：收集中心对纯 HTTP 节点（无 WS 连接）探活时一并取回，用于节点状态页展示版本、以及下发管理指令前判断该节点能否理解对应指令。
