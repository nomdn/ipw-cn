...
const allServers = [
  ...
  ...config.APIBaseURL.IPv6.map((s: ServerConfig) => ({ ...s, type: 'IPv6' }))
];
...
function initServerResults() {
  ...
  config.APIBaseURL.IPv6.forEach((server: ServerConfig) => {
    results.push({
      label: server.label,
      loading: false,
      error: ''
          ...
          <template v-for="(server, index) in serverResults" :key="index">
            <tr v-if="server.ipv6">
              ...
              <td class="table-value">
                <span :class="(server.ipv6?.loss_rate ?? 0) > 0 ? 'loss-warning' : 'loss-ok'">
