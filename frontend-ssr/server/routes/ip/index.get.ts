import { defineEventHandler, getQuery, createError, setResponseStatus, setResponseHeader, getRequestHeader, getRequestIP } from 'h3'
...
export default defineEventHandler(async (event) => {
    ...
    // 3) 确定要查询的 IP：?ip=<ip> 优先，否则查请求方 IP
    const query = getQuery(event)
    let ip = typeof query.ip === 'string' ? query.ip.trim() : ''
    if (!ip) {
        ip = getRequestIP(event, { xForwardedFor: true }) || ''
    }
    ip = ip.trim()
    if (!ip) {
        throw createError({ statusCode: 400, statusMessage: 'Unable to determine IP address' })
    }
